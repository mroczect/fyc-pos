use crate::DbPool;
use crate::error::DbError;
use crate::models::User;
use crate::repositories::traits::UserRepository;
use crate::validation;
use rusqlite::{ErrorCode, params};

pub struct UserRepo {
    pool: DbPool,
}

impl UserRepo {
    pub fn new(pool: DbPool) -> Self {
        Self { pool }
    }

    pub fn create_user_with_conn(
        conn: &rusqlite::Connection,
        username: &str,
        password_hash: &str,
        public_key: &str,
        encrypted_private_key: &str,
    ) -> Result<i64, DbError> {
        validation::validate_username(username)?;
        Self::insert_user(
            conn,
            username,
            password_hash,
            public_key,
            encrypted_private_key,
        )
    }

    pub fn insert_user(
        conn: &rusqlite::Connection,
        username: &str,
        password_hash: &str,
        public_key: &str,
        encrypted_private_key: &str,
    ) -> Result<i64, DbError> {
        let result = conn.execute(
            "INSERT INTO users (username, password_hash, public_key, encrypted_private_key) VALUES (?1, ?2, ?3, ?4)",
            params![username, password_hash, public_key, encrypted_private_key],
        );
        match result {
            Ok(_) => Ok(conn.last_insert_rowid()),
            Err(rusqlite::Error::SqliteFailure(err, _))
                if err.code == ErrorCode::ConstraintViolation =>
            {
                Err(DbError::DuplicateEntry("Username already exists".into()))
            }
            Err(e) => Err(DbError::QueryError(e)),
        }
    }

    pub fn create_user(
        &self,
        username: &str,
        password_hash: &str,
        public_key: &str,
        encrypted_private_key: &str,
    ) -> Result<i64, DbError> {
        <Self as UserRepository>::create_user(
            self,
            username,
            password_hash,
            public_key,
            encrypted_private_key,
        )
    }

    pub fn find_by_username(&self, username: &str) -> Result<Option<User>, DbError> {
        <Self as UserRepository>::find_by_username(self, username)
    }

    pub fn find_by_username_including_inactive(
        &self,
        username: &str,
    ) -> Result<Option<User>, DbError> {
        <Self as UserRepository>::find_by_username_including_inactive(self, username)
    }

    pub fn find_by_id(&self, id: i64) -> Result<Option<User>, DbError> {
        <Self as UserRepository>::find_by_id(self, id)
    }

    pub fn find_by_id_including_inactive(&self, id: i64) -> Result<Option<User>, DbError> {
        <Self as UserRepository>::find_by_id_including_inactive(self, id)
    }

    pub fn deactivate_user(&self, user_id: i64) -> Result<(), DbError> {
        <Self as UserRepository>::deactivate_user(self, user_id)
    }

    pub fn update_password(&self, user_id: i64, new_password_hash: &str) -> Result<(), DbError> {
        <Self as UserRepository>::update_password(self, user_id, new_password_hash)
    }

    fn row_to_user(row: &rusqlite::Row) -> rusqlite::Result<User> {
        Ok(User {
            id: row.get(0)?,
            username: row.get(1)?,
            password_hash: row.get(2)?,
            public_key: row.get(3)?,
            encrypted_private_key: row.get(4)?,
            is_active: row.get::<_, i32>(5)? == 1,
            created_at: row.get(6)?,
            updated_at: row.get(7)?,
        })
    }
}

impl UserRepository for UserRepo {
    fn create_user(
        &self,
        username: &str,
        password_hash: &str,
        public_key: &str,
        encrypted_private_key: &str,
    ) -> Result<i64, DbError> {
        validation::validate_username(username)?;
        let conn = self.pool.get()?;
        Self::insert_user(
            &conn,
            username,
            password_hash,
            public_key,
            encrypted_private_key,
        )
    }

    fn find_by_username(&self, username: &str) -> Result<Option<User>, DbError> {
        let conn = self.pool.get()?;
        Self::find_user_by_username(&conn, username, true)
    }

    fn find_by_username_including_inactive(&self, username: &str) -> Result<Option<User>, DbError> {
        let conn = self.pool.get()?;
        Self::find_user_by_username(&conn, username, false)
    }

    fn find_by_id(&self, id: i64) -> Result<Option<User>, DbError> {
        let conn = self.pool.get()?;
        Self::find_user_by_id(&conn, id, true)
    }

    fn find_by_id_including_inactive(&self, id: i64) -> Result<Option<User>, DbError> {
        let conn = self.pool.get()?;
        Self::find_user_by_id(&conn, id, false)
    }

    fn deactivate_user(&self, user_id: i64) -> Result<(), DbError> {
        let conn = self.pool.get()?;
        let affected = conn.execute(
            "UPDATE users SET is_active = 0, updated_at = datetime('now') WHERE id = ?1",
            params![user_id],
        )?;
        if affected == 0 {
            return Err(DbError::NotFound("User not found".into()));
        }
        Ok(())
    }

    fn update_password(&self, user_id: i64, new_password_hash: &str) -> Result<(), DbError> {
        let conn = self.pool.get()?;
        let affected = conn.execute(
            "UPDATE users SET password_hash = ?1, updated_at = datetime('now') WHERE id = ?2 AND is_active = 1",
            params![new_password_hash, user_id],
        )?;
        if affected == 0 {
            return Err(DbError::NotFound("Active user not found".into()));
        }
        Ok(())
    }
}

impl UserRepo {
    pub fn find_user_by_username(
        conn: &rusqlite::Connection,
        username: &str,
        active_only: bool,
    ) -> Result<Option<User>, DbError> {
        let query = if active_only {
            "SELECT id, username, password_hash, public_key, encrypted_private_key, is_active, created_at, updated_at FROM users WHERE username = ?1 AND is_active = 1"
        } else {
            "SELECT id, username, password_hash, public_key, encrypted_private_key, is_active, created_at, updated_at FROM users WHERE username = ?1"
        };
        let mut stmt = conn.prepare(query)?;
        let mut rows = stmt.query_map(params![username], Self::row_to_user)?;
        match rows.next() {
            Some(user) => Ok(Some(user?)),
            None => Ok(None),
        }
    }

    pub fn find_user_by_id(
        conn: &rusqlite::Connection,
        id: i64,
        active_only: bool,
    ) -> Result<Option<User>, DbError> {
        let query = if active_only {
            "SELECT id, username, password_hash, public_key, encrypted_private_key, is_active, created_at, updated_at FROM users WHERE id = ?1 AND is_active = 1"
        } else {
            "SELECT id, username, password_hash, public_key, encrypted_private_key, is_active, created_at, updated_at FROM users WHERE id = ?1"
        };
        let mut stmt = conn.prepare(query)?;
        let mut rows = stmt.query_map(params![id], Self::row_to_user)?;
        match rows.next() {
            Some(user) => Ok(Some(user?)),
            None => Ok(None),
        }
    }
}
